...
if args.mixhop_h is not None and args.mixhop_id is not None:
    ...
    label = torch.load(os.path.join(BASE_DIR, "ind.n2000-h{}-c5-g{}.ally".format(args.mixhop_h, args.mixhop_id))) # one-hot label
    label = label.argmax(1)
    label = torch.LongTensor(label).to(device)
    edge_mixhop = torch.load(os.path.join(BASE_DIR, "ind.n2000-h{}-c5-g{}.graph".format(args.mixhop_h, args.mixhop_id)))
...
else args.base_dataset is not None:
...
print(f'(sym) kernel_reg0 {kernel_reg0_norm_sym:.2f}; kernel_reg1 {kernel_reg1_norm_sym:.2f}; gnb {gnb_norm_sym:.2f}')
